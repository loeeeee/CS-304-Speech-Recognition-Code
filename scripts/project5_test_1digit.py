...
def make_prediction_single(hmm_inference: HiddenMarkovModelInference, labeled_signals: Dict[str, List[NDArray[np.float32]]]) -> Tuple[List[str], List[str]]:
    overall_bar = tqdm(desc="Overall Progress", total=len(labeled_signals), position=0)
    ground_truth: List[str] = []
    prediction: List[str] = []
    for label_and_signals in labeled_signals.items():
        truth, pred = _make_prediction(hmm_inference, label_and_signals)
        ground_truth.extend(truth)
        prediction.extend(pred)
        overall_bar.update()
    return ground_truth, prediction
...
def main():
    ...
    two_digit_signals = train_dataset.get_all_n_digits(1)
    ...
    plot_confusion_matrix_from_lists(prediction, ground_truth, class_names=list(set(ground_truth)|set(prediction)), title="ConfusionMatrix1Digits", figsize=(16,12))
